 ...
 */
typedef struct Nodeold{
    long ipAddres;//Adrrse id
    int port;//Port
    uint32_t id[IDLENGTH_INT];//id de la node
    struct Nodeold * voisin[NBVOISIN_MAX]; //list de mes voisin
...
void setNodeIdSimple(Nodeold *fromNode, int id);

//serter pour un id
void setNodeId(Nodeold *fromNode, int id[IDLENGTH_INT]);
//Rajoute un voisin dans le tableau des node
void addVoisin(Nodeold *original, Nodeold *voisin);

//set index a la node
void setTabIndex(Nodeold * node, int index);

// Gestion des PIPE

//Creation de fifo
void createFifo(Nodeold *fromNode);
 ...
 */
void send(Nodeold * node , char str[80]);
 ...
 */
void send_find_node(Nodeold *node, char *dest, int * value);

/*
 * Action lors que lon demende de recevoir une node
 *
 */
void receive_send_node(Nodeold * node, int pipe);
